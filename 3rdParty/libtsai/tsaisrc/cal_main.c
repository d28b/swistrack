...
double    Xd[TSAI_MAX_POINTS],
          Yd[TSAI_MAX_POINTS],
          r_squared[TSAI_MAX_POINTS],
          U[7];
...
void      cc_remove_sensor_plane_distortion_from_Xd_and_Yd ()
{
    ...
    for (i = 0; i < cd->point_count; i++) {
    distorted_to_undistorted_sensor_coord (*cc, Xd[i], Yd[i], &Xu, &Yu);
    Xd[i] = Xu;
    Yd[i] = Yu;
...
void      ncc_compute_approximate_f_and_Tz ()
{
    ...
    M = newdmat (0, (cd->point_count - 1), 0, 1, &error);
    if (error) {
    fprintf (stderr, "ncc compute apx: unable to allocate matrix M\n");
    exit (-1);
    }

    a = newdmat (0, 1, 0, 0, &error);
    if (error) {
...
void coplanar_calibration (calibration_data *pcd, calibration_constants *pcc,
                camera_parameters *pcp)
{

	cc=pcc; cd=pcd; cp=pcp;
...
void coplanar_calibration_with_full_optimization (calibration_data *pcd, calibration_constants *pcc,
                camera_parameters *pcp)
{
...
void      noncoplanar_calibration (calibration_data *pcd, calibration_constants *pcc,
                camera_parameters *pcp)
{
...
void      noncoplanar_calibration_with_full_optimization (calibration_data *pcd, calibration_constants *pcc,
                camera_parameters *pcp)
{
...
void      print_error_stats (fp)
    FILE     *fp;
*/
void print_error_stats (FILE* fp, calibration_data *pcd, calibration_constants cc, camera_parameters cp)
  {
  double    mean, stddev, max, sse;

  /* calculate the distorted image plane error statistics for the data set */
  distorted_image_plane_error_stats (pcd,cc,cp,&(mean), &(stddev), &(max), &(sse));
  ...
  undistorted_image_plane_error_stats (pcd,cc,cp,&(mean), &(stddev), &(max), &(sse));
  fprintf (fp, "       undistorted image plane error:\n");
  fprintf (fp,
      "         mean = %.6lf,  stddev = %.6lf,  max = %.6lf  [pix],  sse = %.6lf  [pix^2]\n\n",
        mean, stddev, max, sse);
  /* calculate the undistorted image plane error statistics for the data set */
  object_space_error_stats (pcd,cc,cp,&(mean), &(stddev), &(max), &(sse));
  fprintf (fp, "       object space error:\n");
  fprintf (fp,
        "         mean = %.6lf,  stddev = %.6lf,  max = %.6lf  [mm],  sse = %.6lf  [mm^2]\n\n",
        mean, stddev, max, sse);
  /* calculate the error statistics for the data set */
  normalized_calibration_error (pcd,cc,cp,&(mean), &(stddev));
  fprintf (fp, "       normalized calibration error:  %.6lf\n\n", mean);
  }

void sprint_error_stats (char *str, calibration_data *pcd, calibration_constants cc, camera_parameters cp)
  {
  double        mean, stddev, max, sse;
  char          locstr[255];
  ...
  /* calculate the distorted image plane error statistics for the data set */
  distorted_image_plane_error_stats (pcd,cc,cp,&(mean), &(stddev), &(max), &(sse));
  sprintf (str, "distorted image plane error:\n");
  sprintf (locstr,"  mean = %.6lf,  stddev = %.6lf,  max = %.6lf  [pix],\n  sse = %.6lf  [pix^2]\n\n",
        mean, stddev, max, sse);
  strcat(str,locstr);
  /* calculate the undistorted image plane error statistics for the data set */
  undistorted_image_plane_error_stats (pcd,cc,cp,&(mean), &(stddev), &(max), &(sse));
  sprintf (locstr, "undistorted image plane error:\n");
  strcat(str,locstr);
  sprintf (locstr,"  mean = %.6lf,  stddev = %.6lf,  max = %.6lf  [pix],\n  sse = %.6lf  [pix^2]\n\n",
        mean, stddev, max, sse);
  strcat(str,locstr);
  /* calculate the undistorted image plane error statistics for the data set */
  object_space_error_stats (pcd,cc,cp,&(mean), &(stddev), &(max), &(sse));
  sprintf (locstr, "object space error:\n");
  strcat(str,locstr);
  sprintf (locstr,"  mean = %.6lf,  stddev = %.6lf,  max = %.6lf  [mm],\n  sse = %.6lf  [mm^2]\n\n",
        mean, stddev, max, sse);
  /* calculate the error statistics for the data set */
  strcat(str,locstr);
  normalized_calibration_error (pcd,cc,cp,&(mean), &(stddev));
  sprintf (locstr, "normalized calibration error:  %.6lf\n", mean);
  strcat(str,locstr);
  }
