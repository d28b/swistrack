...
class CBlobIntersectMask : public COperadorBlob {
  ...
  cv::Mat * mTmp;
public:
  CBlobIntersectMask(cv::Mat * mask, cv::Mat * tmp) {
    mMask = mask;
    mTmp = tmp;
  }
...
void THISCLASS::OnStep() {
  ...
  cv::Mat * longTermBackground = mCore->mDataStructureImageBinary.mImage;
  cv::Mat * currentFrame = mCore->mDataStructureInput.mImage;
  if (! currentFrame) {
