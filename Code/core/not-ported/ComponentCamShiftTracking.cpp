...
void THISCLASS::UpdateTrackers(cv::Mat inputImage) {
	for (std::map<int, camshift>::iterator i = mTrackers.begin();
	        i != mTrackers.end(); i++) {
		...
		if (Utility::SquareDistance(box.center, mTracks[i->first].trajectory.back()) > pow(5, (float) 2)) {
			...
			if (dist > 0.01) {
				...
				p.mCompactness = -1;
				p.mOrientation = -1;
				p.mFrameTimestamp = mCore->mDataStructureInput.mFrameTimestamp;
...
void THISCLASS::AddNewTracks(cv::Mat inputImage) {
	...
	for (DataStructureParticles::tParticleVector::const_iterator pIt =
		...
		if ((closestTrack == NULL) || (minSquareDist > mMinNewTrackDistanceSquared)) {
			int id = mNextTrackId++;
			mTracks[id] = Track(id);
			mTracks[id].AddPoint(pIt->mCenter, mCore->mDataStructureInput.mFrameNumber);
			Particle p;
			...
			p.mFrameTimestamp = pIt->mFrameTimestamp;
			...
			cv::Rect start = Utility::RectByCenter(pIt->mCenter.x, pIt->mCenter.y, mInitialWindowSize, mInitialWindowSize);
...
void THISCLASS::FilterTracks() {
	...
	for (DataStructureTracks::tTrackMap::iterator i = mTracks.begin(); i != mTracks.end(); i++) {
		...
		for (DataStructureTracks::tTrackMap::iterator j = mTracks.begin(); j != mTracks.end(); j++) {
	...
	for (set<int>::iterator i = trackIdsToErase.begin(); i != trackIdsToErase.end(); i++) {
		EraseTrack(*i);
	}
}
...
void THISCLASS::ClearTracks() {
	mTracks.clear();
	for (std::map<int, camshift>::iterator i = mTrackers.begin(); i != mTrackers.end(); i++) {
