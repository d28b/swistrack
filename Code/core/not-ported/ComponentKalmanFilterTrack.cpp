...
cv::Point2f THISCLASS::StepFilter(int id, cv::Point2f newMeasurement, cv::Point2f * lastMeasurement) {
  ...
  const cv::Mat y_k = cvKalmanPredict(mKalman[id], 0);
...
void THISCLASS::OnStep() {
  DataStructureTracks::tTrackMap *tracks =
	...
	for (DataStructureTracks::tTrackMap::iterator i = mOutputTracks.begin(); i != mOutputTracks.end(); i++) {
	...
	for (DataStructureTracks::tTrackMap::iterator it = tracks->begin(); it != tracks->end(); it++) {
	  ...
	  for (DataStructureParticles::tParticleVector::iterator it2 = particles->begin();
	    ...
	    if (it->first == it2->mID) {
	      ...
	      cv::Point2f * lastPoint = NULL;
	      if (t.trajectory.size() != 0) {
		lastPoint = &t.trajectory.back();
	      }
	      cv::Point2f newPoint = StepFilter(t.mID, it2->mCenter, lastPoint);
	      Particle p = *it2;
	      p.mCenter = newPoint;
