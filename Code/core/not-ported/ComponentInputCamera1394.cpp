...
void THISCLASS::OnStep() {
	...
	if (mOutputImage->channels() == 1) {
		...
		// Point the input cv::Mat to the camera buffer
		mCamera.getRGB((unsigned char *)mOutputImage->imageData, mOutputImage->channels*mOutputImage->depth / 8*mOutputImage->height*mOutputImage->width);
	}
