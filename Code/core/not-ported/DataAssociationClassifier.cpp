...
void THISCLASS::ExampleToMat(const Example e, cv::Mat * sample) {
...
void THISCLASS::ExampleTableToMat(const THISCLASS::ExampleTable samples, cv::Mat * data, cv::Mat * responses) {
...
bool THISCLASS::IsSameTrack(const Particle & p1, const Particle & p2) {
  ...
  //cv::Mat * sample = cvCreateMat(1, e.size(),CV_32F );
...
THISCLASS::Example THISCLASS::ComputeExample(const Particle & p1, const Particle & p2) {
  ...
  out["particle.1.timestamp"] = Utility::toMillis(p1.mFrameTimestamp);

  out["particle.2.mCenter.x"] = p2.mCenter.x;
  out["particle.2.mCenter.y"] = p2.mCenter.y;
  out["particle.2.timestamp"] = Utility::toMillis(p2.mFrameTimestamp);

  out["distanceInSpacePixels"] = Utility::Distance(p1.mCenter, p2.mCenter);
  out["distanceInTimeSeconds"] = fabs(p1.mFrameTimestamp.Subtract(p2.mFrameTimestamp).GetMilliseconds().ToDouble() / 1000.0);
...
void THISCLASS::Train(const ExampleTable samples) {
  ...
  cv::Mat var_importance = (cv::Mat)forest.get_var_importance();
  if( var_importance ) {
