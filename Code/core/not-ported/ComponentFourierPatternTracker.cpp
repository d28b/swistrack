...
void THISCLASS::OnStart() {
	...
	// Load the images
	mPattern1 = LoadConfigurationColorImage(wxT("Pattern1"), wxT("Pattern 1"));
	mPattern2 = LoadConfigurationColorImage(wxT("Pattern2"), wxT("Pattern 2"));
...
//	output=(2.0/(input.cols*input.rows))*download;
	...
	for (DataStructureParticles::tParticleVector::iterator pIt = mParticles.begin(); pIt != mParticles.end();) {
		Particle & p = *pIt;
		if (p.mFrameNumber<=0) {
			// delete particle
			//printf("deleting");
