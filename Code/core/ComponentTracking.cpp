...
void THISCLASS::OnStart() {
	...
	// initiate mMaxNumber Track classes
	for (int i = 0; i < mMaxNumber; i++) {
		sharedage.push_back(0);
		oldshared.push_back(0);
		restingtraj.push_back(0);
		mTracks[i] = Track(i);
		mTracks[i].AddPoint(cv::Point2f(320, 240), mCore->mDataStructureInput.mFrameNumber);
	}
}
...
void THISCLASS::DataAssociation() {
	...
	for (int id = 0; id < mMaxNumber; id++) {
		double min_dist = 1000000;
		DataStructureParticles::tParticleVector::iterator min_dist_id;

		// find the "closest" particle for object 'id'
		for (DataStructureParticles::tParticleVector::iterator p = particles->begin(); p != particles->end(); p++) {
	...
	// assign particles to existing noise trajectories
	...
	for (p = particles->begin(); p != particles->end(); p++) {
		...
		for (std::map<int, Track>::iterator t = ptargets.begin(); t != ptargets.end(); t++) {
			double dist = Distance(t->second.trajectory.back(), p->mCenter);
	...
	// create new noise trajectories
	int id = ptargets.size();
	for (p = particles->begin(); p != particles->end(); p++) {
		if (p->mID == -1) {
	...
	// this loop goes until all particles/trajectories are known
	int found = 0;
	while (!found) {
		found = 1;
	...
	found = 0;
	for (std::map<int, Track>::iterator t = ptargets.begin(); t != ptargets.end(); t++) {
		double dist = Distance(t->second.trajectory.front(), t->second.trajectory.back());
		if (dist > avg_speed * avg_speed && t->second.trajectory.size() > 5) {
			...
			for (id = 0; id < mMaxNumber; id++) {
				if (shared.at(id)) {
					double dist = Distance(GetPos(id), t->second.trajectory.back());
					if (dist < min_dist) {
						min_dist = dist;
						min_dist_id = id;
					}
...
void THISCLASS::OnStep() {
	...
	// Associate all points with their nearest neighbor
	DataAssociation();
...
*/
void THISCLASS::AddPoint(int i, cv::Point2f p) {
...
void THISCLASS::AddCompetitor(int c) {
	...
	for (std::vector<int>::iterator it = competitors.begin(); it != competitors.end(); it++)
		if (*it == c) exist = 1;
	if (! exist) competitors.push_back(c);
}
...
int THISCLASS::CountSharedTrajectories(std::vector<int>* shared) {
	...
	std::vector<cv::Point2f> redundant;

	int sht = 0;
	for (int i = 0; i < mMaxNumber; i++) {
		cv::Point2f act = GetPos(i);
		int found = 0;
...
cv::Point2f THISCLASS::GetCritPoint(int id) {
...
*/
cv::Point2f THISCLASS::GetPos(int id) {
