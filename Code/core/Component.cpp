...
THISCLASS::Component(SwisTrackCore * stc, const wxString & name):
	mStatus(), mStatusHasError(false), mStatusHasWarning(false), mStarted(false), mHasReloadError(false),
...
Display * THISCLASS::GetDisplayByName(const wxString & name) {
	for (auto display : mDisplays)
		if (display->mName == name) return display;
...
void THISCLASS::ConfigurationWriteXML(wxXmlNode * configuration, ErrorList * xmlerr) {
	...
	// Write configuration
	for (auto entry : mConfiguration) {
		wxXmlNode * node = new wxXmlNode(0, wxXML_ELEMENT_NODE, wxT("parameter"));
		configuration->AddChild(node);
		node->AddAttribute(wxT("name"), entry.first);
		node->AddAttribute(wxT("value"), entry.second);
	}
}
...
cv::Mat THISCLASS::LoadConfigurationImage(const wxString & key, const wxString & label, int flags) {
	...
	cv::Mat image = cv::imread(filename.GetFullPath().ToStdString(), flags);
	if (image.empty()) {
...
cv::Mat THISCLASS::LoadConfigurationGrayscaleImage(const wxString & key, const wxString & label) {
	cv::Mat image = LoadConfigurationImage(key, label, cv::IMREAD_GRAYSCALE);
...
cv::Mat THISCLASS::LoadConfigurationColorImage(const wxString & key, const wxString & label) {
	cv::Mat image = LoadConfigurationImage(key, label, cv::IMREAD_COLOR);
	if (image.empty()) return image;
