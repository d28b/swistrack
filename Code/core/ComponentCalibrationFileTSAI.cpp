...
void THISCLASS::Read3DMatrix(wxXmlNode * node) {
	mSelectedNode = node;
	for (int row = 0; row < 3; row++) {
		for (int col = 0; col < 3; col++) {
			char tmpStr[3] = "aa";
			tmpStr[0] = tmpStr[0] + row;
			tmpStr[1] = tmpStr[1] + col;
			double value = ConfigurationConversion::Double(ReadChildContent(wxString::FromAscii(tmpStr)), 0);
			mCalParam.R.set(row, col, value);
		}
	}
}
...
double THISCLASS::rot(int a, int b) {
	return mCalParam.R.get(a - 1, b - 1);
}
