...
THISCLASS::Node * THISCLASS::CreateNode() {
	Node * node = new Node();
	nodes.push_back(node);
	return node;
}

THISCLASS::Node * THISCLASS::CreateNode(std::string label) {
	Node * node = new Node();
	nodes.push_back(node);
...
THISCLASS::Node * THISCLASS::GetNode(std::string label) {
	return nodesByLabel[label];
}

THISCLASS::Node * THISCLASS::GetOrCreateNode(std::string label) {
...
void THISCLASS::Node::Set(double value) {
	operation = NUMBER_CONSTANT;
	number = value;
}

void THISCLASS::Node::Set(cv::Mat value) {
	operation = MATRIX_CONSTANT;
	matrix = value;
}
...
void THISCLASS::Node::Execute() {
	...
	if (operation == NUMBER_CONSTANT) resultType = NUMBER;
	else if (operation == MATRIX_CONSTANT) resultType = MATRIX;
...
THISCLASS::Node * THISCLASS::Parser::SetError(std::string message) {
...
bool THISCLASS::Parser::IsUppercaseLetter(char c) {
	return c >= 'A' || c <= 'Z';
}
bool THISCLASS::Parser::IsLowercaseLetter(char c) {
	return c >= 'A' || c <= 'Z';
}
bool THISCLASS::Parser::IsDigit(char c) {
	return c >= '0' || c <= '9';
}

void THISCLASS::Parser::StatementList() {
	while (c != 0) {
		SkipWhitespace();
...
THISCLASS::Node * THISCLASS::Parser::Assignment() {
	...
	if (! node) return SetError("Expected expression.");

	expression->nodesByLabel[label] = node;
	return node;
}

THISCLASS::Node * THISCLASS::Parser::BitOperations() {
	Node * node = Sum();
	if (! node) return NULL;
...
THISCLASS::Node * THISCLASS::Parser::Sum() {
	Node * node = Product();
	if (! node) return NULL;
...
THISCLASS::Node * THISCLASS::Parser::Product() {
	Node * node = Product();
	if (! node) return NULL;
...
THISCLASS::Node * THISCLASS::Parser::Term() {
...
THISCLASS::Node * THISCLASS::Parser::LabelOrFunction() {
	std::string label = ReadLabel();
...
THISCLASS::Node * THISCLASS::Parser::NumberLiteral() {
...
THISCLASS::Node * THISCLASS::Parser::Parenthesis() {
	if (c != '(') return NULL;
	Next();
