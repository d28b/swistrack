...
* 	Aded the calc_eval header file to make the whole thing more readable
\*******************************************************************************/
...
/* reference to the global variables defined in cal_main.c */
/*
...
*/
void distorted_image_plane_error_stats (calibration_data *pcd, calibration_constants cc, camera_parameters cp,
		double *mean, double* stddev, double* max, double* sse)
  {
    int       i;
   ...
   for (i = 0; i < pcd->point_count; i++)
   {
      ...
      world_coord_to_image_coord (cc,cp,pcd->xw[i], pcd->yw[i], pcd->zw[i], &Xf, &Yf);

      /* determine the error between the ideal and actual location of the data point    */
      /* (in distorted image coordinates)                 */
      squared_error = SQR (Xf - pcd->Xf[i]) + SQR (Yf - pcd->Yf[i]);
    ...
    *mean = sum_error / pcd->point_count;
    *max = max_error;
   ...
   *stddev = sqrt ((sum_squared_error - SQR (sum_error) / pcd->point_count) / (pcd->point_count - 1));
}
 ...
 */
void undistorted_image_plane_error_stats (calibration_data *pcd, calibration_constants cc, camera_parameters cp,
		double* mean, double* stddev, double* max, double* sse)
{
    int       i;
   ...
   *mean = *stddev = *max = *sse = 0;
   return;
    ...
    for (i = 0; i < pcd->point_count; i++) {
   /* calculate the ideal location of the image of the data point */
   world_coord_to_camera_coord (cc,pcd->xw[i], pcd->yw[i], pcd->zw[i], &xc, &yc, &zc);
   ...
   Xd = cp.dpx * (pcd->Xf[i] - cp.Cx) / cp.sx;
   Yd = cp.dpy * (pcd->Yf[i] - cp.Cy);
...
*/
void object_space_error_stats (calibration_data *pcd, calibration_constants cc, camera_parameters cp,
   double* mean, double* stddev, double* max, double* sse)
{
    int       i;
...
*/
void normalized_calibration_error (calibration_data *pcd, calibration_constants cc, camera_parameters cp,
		double* mean, double* stddev)
  {
  int       	i;

  double    	xc,yc,zc,xc_est,yc_est,zc_est,fu,fv,Xd,Yd,Xu,Yu,
  		distortion_factor,squared_error,sum_error = 0,sum_squared_error = 0;
