...
Pdef = [50 2000 48 2 2 0.9 0.4 1500 1e-25 250 NaN 0 0];
Plen = length(P);
P    = [P,Pdef(Plen+1:end)];
...
%out=zeros(1,ps);
%for prt_index=1:ps
%	out(1, prt_index) = feval(functname,pos(prt_index,:));
%end


out = evaluate(functname,pos );
...
for i=1:me  % start epoch loop (iterations)
 ...
 %    out        = feval(functname,[pos;gbest]);
		out = evaluate(functname,[pos;gbest]);
...
function out = evaluate(functname,pos )

	s = size(pos,1);
	out=zeros(s,1);
	for prt_index=1:s
		out( prt_index,1) = feval(functname,pos(prt_index,:));
	end
end
