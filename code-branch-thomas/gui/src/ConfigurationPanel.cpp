...
THISCLASS::ConfigurationPanel(wxWindow* parent, Component *c):
		wxPanel(parent, -1),
	...
	if (! mIsOpen) {
		mDocument.SetRoot(new wxXmlNode(0, wxXML_ELEMENT_NODE, "swistrackcomponent"));
		return false;
	}

	// Read the main component properties
	wxXmlNode *rootnode=mDocument.GetRoot();
	mComponentDisplayName=GetChildNodeString(rootnode, "name", c->mDisplayName.c_str());
	mComponentDescription=GetChildNodeString(rootnode, "description", "");
	mDefaultDisplay=GetChildNodeString(rootnode, "display", "");

	// Read the list of settings
	wxXmlNode *configurationnode=GetNodeByName(rootnode, "configuration");
	ReadSettings(configurationnode);
...
wxXmlNode *THISCLASS::GetChildNodeByName(wxXmlNode *node, const wxString &name) {
...
wxString THISCLASS::GetChildNodeString(wxXmlNode *parentnode, const wxString &name, const wxString &defvalue) {
	wxXmlNode *node=GetChildNodeByName(parentnode, name);
...
void THISCLASS::ReadParameter(wxXmlNode *node) {
	...
	wxString type=node.GetPropertyString(node, "type");
	ConfigurationParameter *cp=ConfigurationParameter::Create(type);
}

wxString THISCLASS::GetPropertyString(wxXmlNode *node, const wxString &name) {
	if (! node) {return;}
